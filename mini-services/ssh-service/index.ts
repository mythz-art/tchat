...
function bridgeSession(stream: any, roomLabel: string, onEnd: () => void) {
  ...
  const submit = (raw: string) => {
    ...
    if (line.startsWith('/')) {
      ...
      switch ((cmd || '').toLowerCase()) {
        case 'help':
          printLine(`  ${DIM}/nick <name> | /me <action> | /users | /rooms | /history [n|all] | /clear | /quit${R}`)
          break
        case 'nick':
        ...
        case 'history': {
          ...
          const n = wantsAll ? 200 : Math.min(1000, Math.max(1, parseInt(argStr, 10) || 30))
          if (histInFlight) {
  ...
  sock.on('joined', (info: any) => {
    ...
    if (Array.isArray(info.history) && info.history.length) {
      ...
      if (info.hasMore) {
        ...
        printLine(`  ${DIM}${n ? n + ' older message(s) on record — ' : 'older messages on record — '}/history to load more · /history all for everything${R}`)
      }
    }
  ...
  // v3.4 /history all: chain pages until the server reports the floor reached.
  ...
  sock.on('history-page', (d: { messages?: any[]; hasMore?: boolean }) => {
    ...
    if (histHasMore) {
      printLine(`  ${DIM}··· /history for more${histDumpAll ? '' : ' · /history all for everything'}${R}`)
      if (histDumpAll) sock.emit('history', { before: histCursor ?? undefined, limit: 200 })
    } else {
      histDumpAll = false
      printLine(`  ${DIM}··· start of history${R}`)
    }
  })
